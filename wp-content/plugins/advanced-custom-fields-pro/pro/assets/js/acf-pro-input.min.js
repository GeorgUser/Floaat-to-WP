!(function(i) {
  var t = acf.Field.extend({
    ...
    addCollapsed: function() {
      ...
      this.$rows().each(function(t) {
        -1 < e.indexOf(t) && i(this).addClass("-collapsed");
      });
    },
    addUnscopedEvents: function(a) {
      this.on("invalidField", ".acf-row", function(t) {
        var e = i(this);
        a.isCollapsed(e) && a.expand(e);
      });
    },
    initialize: function() {
    ...
    add: function(a) {
      ...
      a = acf.parseArgs(a, { before: !1 });
      var t = acf.duplicate({
        target: this.$clone(),
        append: this.proxy(function(t, e) {
          a.before ? a.before.before(e) : t.before(e),
            e.removeClass("acf-clone"),
            acf.enable(e, this.cid),
            this.render();
        })
      });
    ...
    onUnload: function() {
      var e = [];
      this.$rows().each(function(t) {
        i(this).hasClass("-collapsed") && e.push(t);
      }),
        (e = e.length ? e : null),
  ...
  var a = new acf.Model({
    ...
    key: function(t, e) {
      ...
      return a++, this.set(t + e, a, !0), 1 < a && (t += "-" + a), t;
    },
    load: function(t) {
    ...
    save: function(t, e) {
      var t = this.key(t, "save"),
        a = acf.getPreference(this.name) || {};
      null === e ? delete a[t] : (a[t] = e),
        i.isEmptyObject(a) && (a = null),
        acf.setPreference(this.name, a);
    }
  });
})(jQuery),
  (function(c) {
    var t = acf.Field.extend({
        ...
        var t = this.$popup().html(),
          ...
          r = function(t) {
            return a.filter(function() {
              return c(this).data("layout") === t;
            }).length;
          };
        return (
          e.find("[data-layout]").each(function() {
            var t = c(this),
              e = t.data("min") || 0,
              a = t.data("max") || 0,
              i = t.data("layout") || "",
              n = r(i);
            if (a && a <= n) t.addClass("disabled");
            else if (e && n < e) {
              var o = e - n,
                s = acf.__(
                  "{required} {label} {identifier} required (min {min})"
                ),
                l = acf._n("layout", "layouts", o);
              (s = (s = (s = (s = s.replace("{required}", o)).replace(
                "{label}",
                i
              )).replace("{identifier}", l)).replace("{min}", e)),
                t.append(
                  '<span class="badge" title="' + s + '">' + o + "</span>"
                );
            }
          }),
          (t = e.outerHTML())
      ...
      addCollapsed: function() {
        ...
        this.$layouts().each(function(t) {
          -1 < e.indexOf(t) && c(this).addClass("-collapsed");
        });
      },
      addUnscopedEvents: function(e) {
        this.on("invalidField", ".layout", function(t) {
          e.onInvalidField(t, c(this));
        });
      },
      initialize: function() {
        this.addUnscopedEvents(this),
      ...
      onClickAdd: function(t, e) {
        ...
        var a = null,
          i;
        e.hasClass("acf-icon") && (a = e.closest(".layout")).addClass("-hover"),
          new n({
            target: e,
            targetConfirm: !1,
            ...
            confirm: function(t, e) {
              e.hasClass("disabled") ||
                this.add({ layout: e.data("layout"), before: a });
            },
            cancel: function() {
      ...
      add: function(a) {
        if (
          ((a = acf.parseArgs(a, { layout: "", before: !1 })), !this.allowAdd())
        )
          return !1;
        var t = acf.duplicate({
          target: this.$clone(a.layout),
          append: this.proxy(function(t, e) {
            a.before ? a.before.before(e) : this.$layoutsWrap().append(e),
              acf.enable(e, this.cid),
              this.render();
          })
        });
      ...
      onUnload: function() {
        var e = [];
        this.$layouts().each(function(t) {
          c(this).hasClass("-collapsed") && e.push(t);
        }),
          (e = e.length ? e : null),
          a.save(this.get("key"), e);
      },
    ...
    acf.registerFieldType(t);
    var n = acf.models.TooltipConfirm.extend({
      events: {
  ...
  (function(s) {
    var t = acf.Field.extend({
      ...
      addUnscopedEvents: function(e) {
        this.on("change", ".acf-gallery-side", function(t) {
          e.onUpdate(t, s(this));
        });
      },
      ...
      onClickAdd: function(t, e) {
        if (this.isFull())
          ...
          var a = acf.newMediaPopup({
            ...
            selected: this.val(),
            select: s.proxy(function(t, e) {
              this.appendAttachment(t, e);
            }, this)
          });
      },
      appendAttachment: function(t, e) {
        if (
          ((t = this.validateAttachment(t)),
          !this.isFull() && !this.$attachment(t.id).length)
        ) {
          var a = [
              '<div class="acf-gallery-attachment" data-id="' + t.id + '">',
            ...
            i = s(a);
          if (
            (this.$collection().append(i), "prepend" === this.get("insert"))
          ) {
            var n = this.$attachments().eq(e);
            n.length && n.before(i);
          }
      ...
      validateAttachment: function(t) {
        if (
          ...
          t = t.attributes;
          var e = acf.isget(t, "sizes", "medium", "url");
          null !== e && (t.url = e);
        }
        return t;
      },
      ...
      selectAttachment: function(a) {
        var t = this.$attachment(a);
      ...
      onUpdate: function(t, e) {
        var a = this.$(".acf-gallery-update");
        if (!a.hasClass("disabled")) {
          var i = acf.serialize(this.$sideData());
          a.addClass("disabled"),
            a.before('<i class="acf-loading"></i> '),
            (i.action = "acf/fields/gallery/update_attachment"),
            s.ajax({
              url: acf.get("ajaxurl"),
              data: acf.prepareForAjax(i),
              type: "post",
              dataType: "json",
              complete: function() {
                a.removeClass("disabled"), a.prev(".acf-loading").remove();
              }
            });
        }
      },
